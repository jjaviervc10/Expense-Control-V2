...
export default function Analisis() {
  ...
  // 🔥 Cargar gastos históricos en paralelo desde el backend
  const { gastos: gastosDiarios, loading: loadingDiario } = useGastosPorTipo('diario', 0);
  const { gastos: gastosSemanales, loading: loadingSemanal } = useGastosPorTipo('semanal', 0);
  const { gastos: gastosMensuales, loading: loadingMensual } = useGastosPorTipo('mensual', 0);

  const loading = loadingDiario || loadingSemanal || loadingMensual;

  // Procesar gastos en una sola pasada O(n) - Combinando todas las fuentes
  const chartData = useMemo(() => {
    const map = new Map();
    
    // Combinar todos los gastos de forma eficiente
    ...
    for (const exp of todosLosGastos) {
  ...
  }, [gastosDiarios, gastosSemanales, gastosMensuales]);
