...
export default function Mensual() {
  ...
  const [scanOpen, setScanOpen] = useState(false); // ✅ Estado local para controlar el modal de escaneo
  ...
  } = usePresupuestoActivo("mensual");

  useEffect(() => {
    if (state.expenses.some((e) => e.range === "mensual")) {
      ...
      <div className="relative z-10 min-h-screen bg-transparent p-6">
        ...
        <div className="max-w-3xl mx-auto bg-white bg-opacity-60 shadow-lg rounded-lg mt-10 p-10">
          {loading ? (
            ...
            <BudgetForm tipo="mensual" onSuccess={refetchPresupuesto} />
          ) : (
            <>
              <BudgetTracker tipo="mensual" onResetSuccess={refetchPresupuesto} />
              <main className="max-w-3xl mx-auto py-10">
                <FilterByCategory onChange={setFilterCategory} />
                <ExpenseList tipo="mensual" refresh={refresh} filterCategory={filterCategory} />
                <ExpenseModal />
              </main>
            </>
          )}
